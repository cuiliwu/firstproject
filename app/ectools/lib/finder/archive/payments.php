 ...
 */
class ectools_finder_archive_payments{
    ...
    {
        $payment = $this->app->model('archive_payments');
        if($_POST['payment_id']){
            $sdf = $_POST;
            unset($_POST['_method']);
            if($payment->save($sdf)){
    ...
    {
        $obj_payment = $this->app->model('archive_payments');
        
        $arr_payment = $obj_payment->dump(array('payment_id'=>$row['payment_id']), '*', array('orders' => '*'));
        if ($arr_payment)
