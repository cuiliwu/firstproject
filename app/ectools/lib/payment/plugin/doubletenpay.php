...
如何使用财务通付款？").'</a>';
	...
	public function dopay($payment){
        ...
        $trade_mode = $this->getConf('authtype',__CLASS__);
        $payment['currency'] = "1";    //$order->M_Currency = "1";

        $orderdate = date("Ymd",$payment['t_begin']);    //$order->M_Time
        $payment['M_Amount'] = bcmul($payment['cur_money'],100,0);  //$order->M_Amount
    ...
    public function gen_form(){
      ...
      $tmp_tc_form="<form name='applyForm' method='".$this->fields['postmethod']."' action='http://top.ecclub.cn/recordpayagent.php' target='_blank'>";
      $tmp_db_form="<form name='applyFormAgain' method='".$this->fields['postmethod']."'  action='http://top.ecclub.cn/recordpayagent.php' target='_blank'>";
