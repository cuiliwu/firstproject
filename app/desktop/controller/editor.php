...
EOF;
    ...
    function finder_common(){
         ...
         */
        //因为object控件加入filter以后，会导致url过长。这样的话重新获取filter，就可以了。
        if(isset( $_GET['filter']['get_filter_by_class'] ) && $_GET['filter']['get_filter_by_class'] == 'true' )
        {
            $filter_class   = $_GET['filter']['filter_class'];
            $filter_method  = $_GET['filter']['filter_method'];
            $filter_params  = $_GET['filter']['filter_params'];
            $filter         = kernel::single($filter_class)->$filter_method($filter_params);
            $_GET['filter_class'] = $_GET['filter'];
            $_GET['filter'] = $filter;
        }
