...
class content_finder_indexs
{
	...
	*/
    public function column_edit($row){
        return '<a href="index.php?app=content&ctl=admin_article_detail&act=edit&article_id=' . $row['article_id'] . '&finder_id=' . $_GET['_finder']['finder_id'] . '" target="_blank" >'.app::get('content')->_('编辑').'</a>';
    }
	...
	*/
    public function column_preview($row){
        if($row['platform']=='wap'){
            return '<a href="' .  app::get('wap')->router()->gen_url(array('app'=>'content', 'ctl'=>'wap_article', 'act'=>'index', 'arg0'=>$row['article_id'])) . '" target="_blank" >'.app::get('content')->_('预览').'</a>';
        }else{
