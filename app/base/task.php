...
class base_task{
    ...
    function install_options(){
        if(ECAE_MODE){
           ...
           return array(
                'db_host'=>array('type'=>'text','vtype'=>'required','required'=>true,'title'=>app::get('base')->_('数据库主机'),'default'=>'127.0.0.1'),
    ...
    function post_update($dbinfo) 
    {
        ...
        if(empty($dbver) || $dbver == '0.1'){
            ...
            logger::info('cache expiers update');
        ...
        if($dbver && $dbver<'0.21'){
            $sql = "insert into sdb_base_setting (select SUBSTRING(prefix,9) as app, `key`, value from sdb_base_kvstore where prefix like 'setting/%' );";
            echo 'init sdb_base_setting data...'.PHP_EOL;
