...
class base_view_helper{
    ...
    function function_html_options($params, &$tpl){
        ...
        if (is_array($options))
        {
            ...
            {
                $_html_result .= $this->_tpl_function_html_options_optoutput($tpl, $_key, $_val, $selected);
            }
        }
        else
        {
    ...
    function _tpl_function_html_options_optoutput(&$tpl, $key, $value, $selected){
        if(!is_array($value))
        {
        ...
        {
            $_html_result = $this->_function_html_options_optgroup($tpl, $key, $value, $selected);
        }
        return $_html_result;
    }

    function _function_html_options_optgroup(&$tpl, $key, $values, $selected){
        $optgroup_html = '<optgroup label="' . $this->_tpl_escape_chars($key) . '">' . "\n";
        foreach ($values as $key => $value)
        {
            $optgroup_html .= $this->_tpl_function_html_options_optoutput($tpl, $key, $value, $selected);
        }
