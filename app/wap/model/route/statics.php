...
class wap_mdl_route_statics extends dbeav_model 
{
    ...
    public function searchOptions() 
    {
        ...
        return array_merge($arr, array(
                'static' => app::get('wap')->_('URL规则'),
                'url' => app::get('wap')->_('目标链接'),
            ));
    }//End Function
    ...
    public function insert(&$data) 
    {
        ...
        if($res){
            kernel::single('wap_route_static')->set_dispatch($data['static'], $data);
            kernel::single('wap_route_static')->set_genurl($data['url'], $data);
        }
        return $res;
    }//End Function
    ...
    public function update($data, $filter=array(), $mustUpdate = null)
    {
        ...
        if($res){
            foreach($old_rows AS $row){
                kernel::single('wap_route_static')->del_dispatch($row['static']);
                kernel::single('wap_route_static')->del_genurl($row['url']);
            }
            $rows = $this->getList('*', $filter);
            foreach($rows AS $row){
                kernel::single('wap_route_static')->set_dispatch($row['static'], $row);
                kernel::single('wap_route_static')->set_genurl($row['url'], $row);
            }
        }
        return $res;
    }//End Function
