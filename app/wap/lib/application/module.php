...
class wap_application_module extends base_application_prototype_xml
{
    var $xml='wap.xml';
    var $xsd='wap_content';
    var $path = 'module';
...
/*        if(is_array($this->current['default_menu'])){
        ...
        kernel::single('wap_module_base')->create_site_config();   //生成config文件
    }//End Function
    ...
    {
        $rows = app::get('wap')->model('modules')->getList('id', array('app'=>$app_id, 'update_modified|noequal'=>$this->update_modified));
        foreach($rows AS $row){
            app::get('wap')->model('modules')->delete(array('id'=>$row['id']));
        }
        /*$rows = app::get('wap')->model('menus')->getList('id', array('app'=>$app_id, 'update_modified|noequal'=>$this->update_modified));
        foreach($rows AS $row){
            app::get('wap')->model('menus')->delete(array('id'=>$row['id']));
        }
        $rows = app::get('wap')->model('seo')->getList('id', array('app'=>$app_id, 'update_modified|noequal'=>$this->update_modified));
        foreach($rows AS $row){
    ...
    {
        if($update == true){
            $rows = app::get('wap')->model('modules')->getList('*', array('app'=>$data['app'], 'ctl'=>$data['ctl']));
            if($rows[0]['id'] > 0){
                $data['id'] = $rows[0]['id'];
                $data['path'] = $rows[0]['path'];       //todo: 如果已经安装了此模块，则自定义path和enable值不更新
                $data['enable'] = $rows[0]['enable'];
            }
        }
        $data['update_modified'] = $this->update_modified;
        return app::get('wap')->model('modules')->save($data);
    }//End Function
    ...
    {
        if($update == true){
            $rows = app::get('wap')->model('menus')->getList('id', array('app'=>$data['app'], 'ctl'=>$data['ctl'], 'act'=>$data['act']));
            if($rows[0]['id'] > 0){
                return app::get('wap')->model('menus')->update(array('update_modified'=> $this->update_modified), array('id'=>$rows[0]['id']));
            }
        }
        $data['update_modified'] = $this->update_modified;
        return app::get('wap')->model('menus')->insert($data);
    }//End Function
    ...
    private function insert_seo($data, $update)
    {
        $data['update_modified'] = $this->update_modified;
        if($update == true){
            $rows = app::get('wap')->model('seo')->getList('id', array('app'=>$data['app'], 'ctl'=>$data['ctl'], 'act'=>$data['act']));
            if($rows[0]['id'] > 0){
        unset($data['param']);
                return app::get('wap')->model('seo')->update($data, array('id'=>$rows[0]['id']));
            }
        }
        return app::get('wap')->model('seo')->insert($data);
    }//End Function
    ...
    function clear_by_app($app_id){
        ...
        app::get('wap')->model('modules')->delete(array(
            'app'=>$app_id));
        app::get('wap')->model('menus')->delete(array(
            'app'=>$app_id));
        app::get('wap')->model('seo')->delete(array(
            'app'=>$app_id));
    }
