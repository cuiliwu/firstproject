...
class wap_theme_install{

/*    public function cp_wap_themes_dir(){
        $themeDir = app::get('wap')->app_dir . '/init_wap_themes';
        $destDir =  ROOT_DIR . '/wap_themes/';
        if($isdir = utils::mkdir_p($destDir)){
            utils::cp($themeDir,  $destDir);
        }else{
            logger::info('mkdir wap_themes on root_dir failure');
            echo "Error: 根目录下创建wap_themes目录失败!\n";
            return false;
        }
    }*/
    ...
    public function check_install(){
        ...
        while (false !== ($entry = $d->read())) {
            ...
            if(is_dir(WAP_THEME_DIR . '/' . $entry)&&!ECAE_MODE){
                $themeData = app::get('wap')->model('themes')->select()->where('theme = ?', $entry)->instance()->fetch_row();
                if(empty($themeData)){
                    $this->init_theme($entry);
                }
            }
            if(!kernel::single('wap_theme_base')->get_default()){
                kernel::single('wap_theme_base')->set_default($entry);
            }
        }
        $d->close();
    }//End Function
    ...
    public function remove_theme($theme){
        ...
        kernel::single('wap_theme_base')->delete_theme_widgets($theme);        //todo:删除模板挂件
    ...
    public function flush_theme($theme){
        app::get('wap')->model('themes')->delete(array('theme'=>$theme));
        kernel::single('wap_theme_tmpl')->delete_tmpl_by_theme($theme);
        kernel::single('wap_theme_widget')->delete_widgets_by_theme($theme);
    }//End Function
    ...
    public function install($file, &$msg){
        ...
        if(is_dir(WAP_THEME_DIR.'/'.trim($filename))){
           $filename=time();
        }
        $sDir=$this->__build_dir(str_replace('\\','/',WAP_THEME_DIR.'/'.trim($filename)));
        if($tar->openTAR($file['tmp_name'], $sDir)){
            if($tar->containsFile('theme.xml')) {
                ...
                $obj = app::get('wap')->model('themes_file');
                ...
                if(!$config=$this->init_theme(basename($sDir),'','upload')){
                    ...
                    $msg=app::get('wap')->_('ecclub模板包创建失败');
                    return false;
                }

                kernel::single('wap_theme_base')->install_theme_widgets($filename);        //todo:安装模板挂件

                foreach(kernel::servicelist('wap_theme.post_install') AS $service){
                ...
                $msg = app::get('wap')->_('不是标准的ecclub模板包');
                return false;
            }
        }else{
            $msg = app::get('wap')->_('模板包已损坏，不是标准的ecclub模板包').$file['tmp_name'];
            return false;
        }
    }
    ...
    public function init_theme($theme, $replaceWg=false, $upload='', $loadxml=''){
        ...
        $sDir=WAP_THEME_DIR.'/'.$theme.'/';
        $xml = kernel::single('wap_utility_xml');
        $loadxml_content = kernel::single('wap_theme_file')->get_xml_content($theme, $sDir, $loadxml);
        if($loadxml_content){
        ...
        if(ECAE_MODE){
            $model_file = app::get('wap')->model('themes_file');
            $filter = array('theme'=>$theme);
            $file_rows = $model_file->getList('*',$filter);
            $http = kernel::single('base_httpclient');
            $http->set_timeout(10);
            $storager = kernel::single('base_storager');
            $theme_dir = WAP_THEME_DIR.'/'.$theme;
            if(!is_dir($theme_dir)){
        ...
        if($upload=="upload" && $config['theme']['id']['value']){
            ...
            if($this->file_rename(WAP_THEME_DIR.'/'.$theme,WAP_THEME_DIR.'/'.$config['theme']['id']['value'])){
                $sDir=WAP_THEME_DIR.'/'.$config['theme']['id']['value'];
        ...
        $theme_objs = app::get('wap')->model('themes')->dump(array('theme'=>$aTheme['theme']));
        if($theme_objs){
        ...
        $aNumber= kernel::single('wap_theme_widget')->count_widgets_by_theme($theme);
        ...
        if($insertWidgets && count($aWidgets)>0){
            foreach($aWidgets as $k=>$wg){
                kernel::single('wap_theme_widget')->insert_widgets($wg);
            }
        }

        kernel::single('wap_theme_tmpl')->install($theme);
        if(!kernel::single('wap_theme_base')->update_theme($aTheme)){
            return false;
        }else{
            kernel::single('wap_theme_base')->update_theme_widgets($theme);    //todo:升级模板挂件
            kernel::single('wap_theme_base')->set_theme_views($theme, $views);
            kernel::single('wap_theme_base')->set_last_modify();
            return $aTheme;
        }
    }
    ...
    private function __remove_db_theme($theme) {
        $filter = array('theme'=>$theme);
        return app::get('wap')->model('themes_file')->delete($filter);
    }
                ...
                if ($sItem!='.' && $sItem!='..'){
                    if(is_dir($sDir.'/'.$sItem)){
                        ...
                        if(!unlink($sDir.'/'.$sItem)){
                            trigger_error(app::get('wap')->_('因权限原因，模板文件').$sDir.'/'.$sItem.app::get('wap')->_('无法删除'),E_USER_NOTICE);
                        }
                    }
                }
            }
    ...
    public function get_file_ext($file_name){
        $ftype = array(
            'html' => app::get('wap')->_('模板文件'),
            'gif'  => app::get('wap')->_('图片文件'),
            'jpg'  => app::get('wap')->_('图片文件'),
            'jpeg' => app::get('wap')->_('图片文件'),
            'png'  => app::get('wap')->_('图片文件'),
            'bmp'  => app::get('wap')->_('图片文件'),
            'css'  => app::get('wap')->_('样式表文件'),
            'js'   => app::get('wap')->_('脚本文件'),
            'xml'  => app::get('wap')->_('theme.xml'),
            'php'  => app::get('wap')->_('模板挂件'),
        );
    ...
    public function initthemes(){
        if ($dh = opendir(WAP_THEME_DIR)){
            while (($file = readdir($dh)) !== false){
                if(substr($file,-4,4)=='.tgz'){
                    $theme_file['tmp_name'] = WAP_THEME_DIR.'/'.$file;
                    $theme_file['name'] = $file;
                    $theme_file['type'] = 'application/octet-stream';
                    $theme_file['error'] = '0';
                    $theme_file['size'] = filesize(WAP_THEME_DIR.'/'.$file);
