...
class wap_theme_tmpl_dbsave
{
    ...
    public function install($theme)
    {
        ...
        if(file_exists(WAP_THEME_DIR.'/'.$theme.'/cart.html')){
            ...
            if(!file_exists(WAP_THEME_DIR.'/'.$theme.'/order_index.html')){
    ...
    public function get_themes_file_id($filter){
        if($file_id = app::get('wap')->model('themes_file')->getList('id',$filter)){
            return $file_id['0']['id'];
        }else{
            return false;
        }
    ...
    public function insert_tmpl($data,&$msg)
    {
        ...
        $data['version'] = time();
        $tmpl=app::get('wap')->model('themes_tmpl')->dump(array(tmpl_path=>$data['tmpl_path']),'tmpl_path');
        if($tmpl){
            $msg=app::get('wap')->_('文件名称重复,请检查!');
            return false;
        }
        ...
        $obj_themes_file = app::get('wap')->model('themes_file');
        ...
        if($this->insert($data)){
            ...
            // app::get('wap')->model('themes_file')->save($data,$filter);
            $msg=app::get('wap')->_('添加成功');
            return true;
        }
    ...
    public function insert_themes_file($data){
        if($file_id = app::get('wap')->model('themes_file')->save($data)){
            return $file_id;
        }else{
            return false;
        }
    ...
    public function copy_tmpl($tmpl, $theme)
    {
        ...
        $fileObj = app::get('wap')->model('themes_file');
        ...
        $flag = true;
        $obj = kernel::single('wap_explorer_file');
        $obj->set_theme($theme);
