...
class wap_theme_base
{

    public function set_last_modify()
    {
        return app::get('wap')->setConf('wap_theme_last_modify', time());
    }//End Function

    public function get_last_modify()
    {
        return app::get('wap')->getConf('wap_theme_last_modify');
    }//End Function
    ...
    public function set_default($theme){
        ...
        app::get('wap')->model('themes')->update(array('is_used'=>'false'), array());
        app::get('wap')->model('themes')->update(array('is_used'=>'true'), array('theme'=>$theme));
        return app::get('wap')->setConf('current_theme', $theme);
    }
    ...
    {
        return app::get('wap')->getConf('current_theme');
    }//End Function

    public function update_theme($aData)
    {
        return app::get('wap')->model('themes')->save($aData);
    }//End Function

    public function set_theme_style($theme, $style)
    {
        return app::get('wap')->setConf('theme_style.'.$theme, $style);
    }//End Function

    public function get_theme_style($theme)
    {
        return app::get('wap')->getConf('theme_style.'.$theme);
    }//End Function

    public function set_theme_cache_version($theme)
    {
        return app::get('wap')->setConf('theme_cache_version.'.$theme, time());
    }//End Function

    public function get_theme_cache_version($theme)
    {
        return app::get('wap')->getConf('theme_cache_version.'.$theme);
    }//End Function

    public function get_view($theme)
    {
        if ($handle=opendir(WAP_THEME_DIR.'/'.$theme)){
            $views = array();
            while(false!==($file=readdir($handle))){
                if ($file{0}!=='.' && $file{0}!=='_' && is_file(WAP_THEME_DIR.'/'.$theme.'/'.$file) && (($t=strtolower(strstr($file,'.')))=='.html' || $t=='.htm')){
                    $views[] = $file;
                }
            }
            closedir($handle);
    ...
    public function get_basic_config($theme){
        $basic_config='on';
        $path = WAP_THEME_DIR.'/'.$theme;
        if(!is_dir($path)&&!ECAE_MODE)  return array();
        $workdir = getcwd();
        chdir($path);
        $xml = kernel::single('wap_utility_xml');
        $content = kernel::single('wap_theme_file')->get_theme_xml($theme, 'theme.xml');
    ...
    {
        $aConfig = app::get('wap')->model('themes')->select()->columns(array('config'))->where('theme = ?', $theme)->instance()->fetch_one();
    ...
    {
        return app::get('wap')->model('themes')->select()->where('theme = ?', $theme)->instance()->fetch_row();
    }//End Function

    public function install_theme_widgets($theme)
    {
        foreach(kernel::servicelist('wap_theme_content_detector') AS $service){
            $service->update($theme);
            base_kvstore::instance('wap_themes')->store('theme_last_modified'.get_class($service).$theme, $service->last_modified($theme));
        }
    }//End Function
    ...
    public function update_theme_widgets($theme, $force=false)
    {
        foreach(kernel::servicelist('wap_theme_content_detector') AS $service){
            ...
            if( $force
            || base_kvstore::instance('wap_themes')->fetch('theme_last_modified'.get_class($service).$theme, $modified) === false
            || $last_modified != $modified ){
                logger::info('autofix wap_theme widgets...');
                $service->update($theme);
                base_kvstore::instance('wap_themes')->store('theme_last_modified'.get_class($service).$theme, $last_modified);
            }
        }
    }//End Function
    ...
    public function delete_theme_widgets($theme)
    {
        foreach(kernel::servicelist('wap_theme_content_detector') AS $service){
            ...
            base_kvstore::instance('wap_themes')->delete('theme_last_modified'.get_class($service).$theme);
        }
    }//End Function

    public function set_theme_views($theme, $views)
    {
        return app::get('wap')->setConf('theme_views_' . $theme, $views);
    }//End Function

    public function get_theme_views($theme)
    {
        return app::get('wap')->getConf('theme_views_' . $theme);
    }//End Function
