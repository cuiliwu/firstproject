...
class wap_router implements base_interface_router
{
        ...
        if(!is_array($this->sitemap)){
            ...
            if(is_array($sitemap_config)){
                ...
                if(!kernel::single('wap_module_base')->write_config($sitemap_config)){
                    logger::error('Error: sitemap can\'t save to kvstore');      //todo：如果写入失败，记录于系统日志中，前台不报错，保证网站运行正常
                }
            }else{
                //if false
        ...
        if(isset($this->urlmap[$params['app'].':'.$params['ctl']])){
            ...
            if(!isset($this->__gen_url_array[$gen_key])){
                ...
                if(!isset($this->__site_router_service[$app])){
                    $app_router_service = kernel::service('wap_router.' . $app);
    ...
    public function default_gen_url($params=array())
    {
        ...
        if(!isset($this->__base_url[$full])){
            $this->__base_url[$full] = app::get('wap')->base_url((($full=='true')?true:false));
        }
    ...
    public function http_status($code)
    {
        ...
        $c = 'errorpage.'.$code;
        $msg = kernel::single('wap_errorpage_get')->getConf( $c );;
        if( $msg ) kernel::single('wap_controller')->splash( 'failed',null,$msg);
    ...
    public function dispatch($query){
        //如果未开启手机平台 则跳转PC端
        if(!$this->app->getConf('wap.status') || $this->app->getConf('wap.status')=='false') {
            $this->_response->set_redirect(kernel::base_url(1))->send_headers();
         }
        ...
        if(count($post)>0 || $config_page_cache==='false' || !cachemgr::get($page_key, $page)){
        ...
        // echo "<pre>";print_r($this->sitemap);exit;
            ...
            $service = kernel::service('wap_router.' . $route_value[0]);
            ...
            if(count($post)==0 && $this->_response->get_http_response_code()==200 && $this->has_page_cache_control()===true && $config_page_cache==='true'){
                $page_cache = true;
                ...
                $theme_last_modifty = kernel::single('wap_theme_base')->get_last_modify();     //todo:记录theme最后更新时间，避免无模板情况cache问题
    ...
    {
        if($val = kernel::single('wap_route_static')->get_dispatch($query)){
            if($val['enable'] == 'true'){
                return $val['url'];
            }
        }
    ...
    {
        if($val = kernel::single('wap_route_static')->get_genurl($url)){
            return $val;
        }
        return $url;
    }//End Function
