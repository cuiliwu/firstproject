    ...
    final public function page($view, $no_theme=false, $app_id=null)
    {
        ...
        if (!$params['response_type']){
           ...
           $this->extract_widgets_css($html);
            $html = str_replace('<%wap_widgets_css%>', app::get('wap')->base_url(1).'widgetsproinstance-get_css-'.$current_theme.'-'.base64_encode($tmpl_file).((app::get('site')->getConf('base.enable_site_uri_expanded') == 'true')?'.'.app::get('site')->getConf('base.site_uri_expanded_name'):''), $html);
