...
class operatorlog_goods
{
    ...
    function batchUpdateInt( $goods_id, $updateName, $updateValue , $tableName = '' ){
        ...
        if($updateName == 'cat_id'){
          $catName = app::get('b2c')->model('products')->db->selectrow('SELECT cat_name FROM sdb_b2c_goods_cat WHERE cat_id = '.intval($updateValue) );
