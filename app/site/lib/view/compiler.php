...
class site_view_compiler
{
    ...
    public function __siet_parse_widget_instance($widget, &$wg_compiler, $type)
    {
        ...
        if(file_exists($func_file)||ECAE_MODE){
            $get_php_code = kernel::single('site_theme_file')->get_func_phpcode($cur_theme, $func_file, $widget['app']);
            ...
            $return .= 'if (WITHOUT_CACHE!==true) logger::info("widget cache miss => app:'.$widget['app'].',theme:'.$widget['theme'].',type:'.$widget['widgets_type'].'.");';
            ...
            $return .= 'else {if (WITHOUT_CACHE!==true) logger::info("widget cache hit => app:'.$widget['app'].',theme:'.$widget['theme'].',type:'.$widget['widgets_type'].'.");}';
        ...
        $content = kernel::single('site_theme_file')->get_widgets_content($cur_theme, $tpl, $widget['app']);
