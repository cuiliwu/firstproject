...
class site_theme_widget
{
    ...
    public function widgets_info($name, $app, $theme, $key=null)
    {
       ...
       if($name&&$widgets_dir = $this->widgets_exists($name, $app, $theme)){
            if(ECAE_MODE){
                ...
                $widgets_code = kernel::single('site_theme_file')->get_widgets_code($theme, $app, $widgets_dir);
                eval('?>'.$widgets_code);
           }else{
    ...
    public function widgets_config($name, $app, $theme)
    {
        if(empty($theme)){
            ...
            $data = ecos_cactus('site','theme_widget_widgets_config_empty',$name, $data, $app);
        }else{
    ...
    public function get_libs($theme)
    {
        ...
        foreach($data AS $val){
            if($val['app']==''){
                ...
                $widgetsLib2 = ecos_cactus('site','theme_widget_widgets_get_libs_notype',$info2, $val, $widgetsLib2);
            }
        }
    ...
    public function get_libs_extend($theme, $type='')
    {
        if($theme){
            ...
            $data = app::get('site')->model('widgets')->select()->where('theme = ?', '')->instance()->fetch_all();
        }
        $widgetsLib = array();
        $order=array();
        if($type==null){
    ...
    public function admin_load($file, $slot, $id=null, $edit_mode=false){
        ...
        foreach($rows as $widgets){
            ...
            $widgets_box=preg_replace($replacement,array_fill(0,count($replacement),'xecclub'),$widgets_box);
