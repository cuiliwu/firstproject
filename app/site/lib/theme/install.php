...
class site_theme_install{
        ...
        if($tar->openTAR($file['tmp_name'], $sDir)){
            if($tar->containsFile('theme.xml')) {
                ...
                if(!$config=$this->init_theme(basename($sDir),'','upload')){
                    ...
                    $msg=app::get('site')->_('ecclub模板包创建失败');
                    return false;
                }
                ...
                $msg = app::get('site')->_('不是标准的ecclub模板包');
                return false;
            }
        }else{
            $msg = app::get('site')->_('模板包已损坏，不是标准的ecclub模板包').$file['tmp_name'];
            return false;
        }
    }
    ...
    public function init_theme($theme, $replaceWg=false, $upload='', $loadxml=''){
        ...
        $loadxml_content = kernel::single('site_theme_file')->get_xml_content($theme, $sDir, $loadxml);
        if($loadxml_content){
