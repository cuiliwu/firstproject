<?php
/*>您的系统不支持php<div style
 */
//pathinfo 测试
if($_GET['servera']){
	 foreach($_SERVER as $group=>$items){
            $body.="<tbody><tr class=\"title\"><td colspan=\"3\"><div></div></td></tr><tbody>
			<tr{$rowOpt}><th>{$group}</th><td colspan=\"2\">{$items}</td></tr>";
	 }
	 $html=<<<EOF
...
<a href="http://www.ecclub.cn" target="_blank" style="color:#fff;font-size:12px">ecclub&copy;2010</a>
</center>
 </body>
</html>
EOF;
...
if($path_info){
	...
	EXIT;
}elseif(isset($_GET['check_path_info'])){
...
function check_pathinfo(){
	...
	$uri = $_SERVER['REQUEST_URI'].'/dfdsfdsfdf?check_path_info=1';
    	...
    	return (strpos($content[1],'SUPPORT_PATHINFO')!==false)?true:false;
...
class mdl_serverinfo{
    ...
    function test_server_req(&$score){
    	...
    	$items["pathinfo的支持"] = array(
            'value'=>$pathinfo?'支持':'不支持',
            'result'=>$pathinfo,
        );
...
/*- img-end -*/
    ...
    }elseif(isset($_GET['phpinfo'])&&!file_exists(dirname(__FILE__)."/../config/config.php")){
        phpinfo();
...
<hr />
<div>
    <div style="float:right;"><a href="?servera=ture" target='_blank'>SERVER测评</a>|$phpinfo<a href="?download=true">下载本文件</a></div>
